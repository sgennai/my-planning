...
import { DAY_NAMES_LONG, WEEKLY_RESET_PHASES, WEEKLY_RESET_TOTAL_SECONDS } from '../storage/data';
import { pad, hexToRgba, ColorPickerExtended } from './helpers';
import { startOfDay, startOfWeek, addDays, toMinutes, formatDateShort, makeOverrideKey } from '../helpers/calendar-utils';
import { wmoIcon } from '../helpers/weather';
import { exportDataBlob, importDataBlob, loadData } from '../storage/db';
import { App } from '../main-app';
import { RoutineManagerModal } from '../routine/Routine';
...
// ═════════════════════════════════════════════════════════════
export function WeatherStrip({ settings, cache, refreshing, error, dayTab, now, onChangeDayTab, onRefresh, onRequestGeo }) {
...
export function CaptureBar({ inbox, onAdd, onOpenInbox }) {
...
// ═════════════════════════════════════════════════════════════
export function InboxModal({ inbox, onClose, onDelete, onAdd }) {
...
// ═════════════════════════════════════════════════════════════
export function ElsewhereToggles({ elsewhere, onToggle }) {
  return (
...
// ═════════════════════════════════════════════════════════════
export function ElsewhereSuggestions({ routine, overrides, elsewhere, today, onMove, onDismiss }) {
...
// ═════════════════════════════════════════════════════════════
export function TodosPane({ todos, scheduledBlocks, onAdd, onUpdate, onDelete }) {
  const [value, setValue] = useState('');
...
// ═════════════════════════════════════════════════════════════
export function TodoDurationPrompt({ drop, todo, onConfirm, onCancel }) {
...
export function SettingsModal({ calendars, icsCache, icsRefreshing, onUpdate, onRefresh, weather, onUpdateWeather, onRequestGeo, lunchSlot, onSetLunchSlot, onClose,
  ...
  const [syncUrl, setSyncUrl] = useState(calendars.syncUrl || '');
  const [syncSecret, setSyncSecret] = useState(calendars.syncSecret || '');
  ...
  const handleImport = async (e) => {
    ...
    if (confirm('This will wipe all existing data and restore from the backup file. Are you sure?')) {
      try {
        await importDataBlob(e.target.files[0]);
  ...
  const renderStatus = (key) => {
    ...
    if (cache.error) {
      const msg = cache.error.includes('429') ? 'Google rate limit — stale data will be served until quota resets' : cache.error;
        ...
        {activeTab === 'routines' ? (
          ...
          <div className="sm-body">
            ...
            <div className="sm-section">
              <div className="sm-eyebrow">Data & Storage (Local IndexedDB)</div>
              <div className="sm-card">
                <div className="sm-hint" style={{ marginBottom: 12 }}>
                  Your data is stored exclusively on this device. Export a backup regularly.
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
                  <button className="sm-btn primary" onClick={handleExport}>Download Backup (JSON)</button>
            ...
            {/* ── CLOUDFLARE ── */}
            <div className="sm-section">
              <div className="sm-eyebrow">Cloudflare Infra</div>
              <div className="sm-card">
                <div className="sm-field">
                  <div className="sm-field-label">Proxy Worker URL</div>
                  <input type="text" className="sm-input" value={proxyUrl}
                    placeholder="https://your-worker.your-subdomain.workers.dev"
                    onChange={e => setProxyUrl(e.target.value)} />
                  <div className="sm-hint">Relays ICS feeds and Todoist requests.</div>
                </div>
                <div className="sm-field">
                  <div className="sm-field-label">Sync Worker URL (D1)</div>
                  <input type="text" className="sm-input" value={syncUrl}
                    placeholder="https://my-planning-sync.your-subdomain.workers.dev"
                    onChange={e => setSyncUrl(e.target.value)} />
                </div>
                <div className="sm-field">
                  <div className="sm-field-label">Sync Secret</div>
                  <input type="password" className="sm-input" value={syncSecret}
                    placeholder="Enter Worker SYNC_SECRET"
...
// ═════════════════════════════════════════════════════════════
export function FridayReviewLauncher({ now, weeklyResets, onLaunch }) {
...
export function ResetPrompt({ prompt, index, value, scorecardValue, storyValue, phaseColor, onUpdate }) {
  return (
    <div>
